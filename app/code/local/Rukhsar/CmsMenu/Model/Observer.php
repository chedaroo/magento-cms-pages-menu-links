 ...
 *
 */
class Rukhsar_CmsMenu_Model_Observer
{

    public function saveCmsMenu($observer)
    {
        $isEnabled = Mage::getStoreConfig('rukhsar_cmsmenu/general/enabled');
        if($isEnabled) {
            try {
                ...
                $pageId = $page->getId();
                $cmsMenu = Mage::getModel('rukhsar_cmsmenu/cmsmenu')->load($pageId, 'cms_page_id');
                if (!$cmsMenu || ($cmsMenu && !$cmsMenu->getId())) {
                    $cmsMenu = Mage::getModel('rukhsar_cmsmenu/cmsmenu');
                }
                $level = Mage::helper('rukhsar_cmsmenu')->getTreeLevel($post['child_of']);
                ...
                $cmsMenu->save();
                $flushCache = Mage::getStoreConfig('rukhsar_cmsmenu/general/cache');
                if($flushCache) {
                    Mage::app()->getCacheInstance()->flush();
                }
            } catch (Exception $e) {
                Mage::log($e->getMessage(), null, 'rukhsar-cmsmenu-error.log');
            }
